...
related: [wiki/concepts/thread-safety, wiki/concepts/locking, wiki/concepts/eventual-consistency, wiki/topics/concurrent-programming, wiki/sources/java-concurrency-in-practice]
---
...
- **The connection to distributed systems:** DDIA's treatment of replication lag and stale reads (see [[wiki/concepts/eventual-consistency]]) is the same visibility problem, scaled to distributed nodes. In both cases, writes are not immediately visible to all readers; in both cases, explicit synchronization or consistency protocols are needed to guarantee recency. The analogy runs surprisingly deep: volatile ≈ read-your-own-writes; synchronized ≈ linearizability.

## Related

- [[wiki/concepts/thread-safety]] — visibility is one of the two properties synchronization must provide
- [[wiki/concepts/locking]] — locking provides visibility as a side effect of synchronization boundaries
- [[wiki/concepts/eventual-consistency]] — the distributed analogue: stale reads due to replication lag mirror stale reads due to cache/reordering
